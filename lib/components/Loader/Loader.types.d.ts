...
export type LoaderType = 'pacman' | 'orbit' | 'circle' | 'loading-text' | 'sync-bars' | 'sync-dots' | 'rises' | 'atomic' | 'pulse' | 'classic-buffer' | 'orbit-rotate' | 'clock-wise' | 'cube-box' | 'beat' | 'line-bar';
export interface LoaderTypeProps {
    /**
     *### Loader Type :
     ```typescript
     type: "pacman" | "orbit" | "circle" | "loading-text" | "sync-bars" | "sync-dots" | "rises" | "atomic" | "pulse" | "classic-buffer" | "orbit-rotate" | "clock-wise" | "cube-box" | "beat" | "line-bar"
     ```
    */
    type?: LoaderType;
    rd?: number | string;
...
export interface getLoaderStyleVarsProps extends DESTRUCTURING_getLoaderStyleVarsProps, CLASSNAME, STYLE {
}
type DESTRUCTURING_LoaderProps = {
    active?: boolean;
} & DESTRUCTURING_getLoaderStyleVarsProps & Omit<React.HTMLAttributes<HTMLSpanElement>, 'color'>;
export type LoaderProps = LoaderTypeProps & DESTRUCTURING_LoaderProps & ChildrenProps;
export declare const getLoaderStyleVars: (order: 'root', { size, c }: DESTRUCTURING_getLoaderStyleVarsProps) => {
    [key: string]: string;
};
/**
 *### EXTENDS_LoaderProps
